  ...
  async execute(interaction) {
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
        .setColor(0x5865F2)
        .setTitle('⏳ Backup Starting...')
        .setDescription(categoryFilter
        ? `Backing up: **${categoryFilter.join(', ')}**`
        : 'Backing up **all categories**...')
        .setFooter({ text: 'This may take a while — the bot will update this message when done' }),
      ],
    });
    ...
    try {
      ...
      .setColor(0x57F287)
      .setTitle(`✅ Backup Complete — ${results.length} categor${results.length === 1 ? 'y' : 'ies'}`)
      .setTimestamp();
